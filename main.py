...
def get_base_path():
    """Return the base path for resource lookup, handling both source and frozen (PyInstaller) modes."""
...
class API:
    ...
    def get_logs(self):
        """Get all log records"""
        global log_records
        return {"status": "success", "logs": log_records}
    ...
    def open_logs_window(self):
        ...
        def copy_all_logs():
            try:
                ...
                status_label.configure(
                    text="Copied all logs to clipboard!", text_color="green"
                )
            except ImportError:
                status_label.configure(
                    text="Please install pyperclip: pip install pyperclip",
                    text_color="orange",
                )
            except Exception as e:
        ...
        def refresh_logs():
            ...
            text_box.insert("1.0", "\n".join(log_records))
            status_label.configure(
                text=f"Showing {len(log_records)} log entries", text_color="white"
            )
        ...
        def save_logs():
            try:
                ...
                with open(filename, "w", encoding="utf-8") as f:
                    f.write("\n".join(log_records))
                status_label.configure(
                    text=f"Logs saved to {filename}", text_color="green"
                )
            except Exception as e:
        ...
        title_label = ctk.CTkLabel(
            main_frame, text="DeepSeek Application Logs", font=("Inter", 16, "bold")
        )
        ...
        status_label = ctk.CTkLabel(
            main_frame,
            text=f"Showing {len(log_records)} log entries",
            font=("Inter", 12),
        )
        ...
        refresh_btn = ctk.CTkButton(
            button_frame, text="Refresh", command=refresh_logs, width=100
        )
        refresh_btn.pack(side="left", padx=5)

        copy_btn = ctk.CTkButton(
            button_frame, text="Copy All", command=copy_all_logs, width=100
        )
        copy_btn.pack(side="left", padx=5)

        save_btn = ctk.CTkButton(
            button_frame, text="Save to File", command=save_logs, width=100
        )
        save_btn.pack(side="left", padx=5)

        clear_btn = ctk.CTkButton(
            button_frame, text="Clear", command=clear_logs, width=100, fg_color="red"
        )
    ...
    def take_screenshot(self):
        ...
        if platform.system() != "Windows" or not ctypes:
            return {
                "status": "error",
                "message": "Native screenshot only supported on Windows",
            }
        ...
        try:
            ...
            elif hasattr(self._window, "_window") and hasattr(
                self._window._window, "hwnd"
            ):
...
class DeepSeekApp:
    ...
    def run(self):
        ...
        webview.start(
            private_mode=False, storage_path="./data", debug=not self.release_mode
        )
...
def main():
    ...
    global titlebar_preference
    titlebar_preference = (
        "dark" if args.dark_titlebar else ("light" if args.light_titlebar else "auto")
    )
